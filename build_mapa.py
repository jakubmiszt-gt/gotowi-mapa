...
function rysujMape(k){
  if(!map) return;
  warstwa.clearLayers();
  swieze.filter(s => k==='all' || s.prio===k).forEach(s => {
...
# index.html jako przekierowanie, zeby nie bylo dwoch kopii do utrzymania
idx = OUT.parent / 'index.html'
idx.write_text(
    '<!DOCTYPE html><html lang="pl"><head><meta charset="utf-8">'
    '<meta http-equiv="refresh" content="0; url=' + OUT.name + '">'
    '<link rel="canonical" href="' + OUT.name + '">'
    '<title>Monitoring regionalny | Gotowi Teraz</title></head>'
...
print('pliki:', OUT.name, OUT.stat().st_size, 'B |', idx.name, idx.stat().st_size, 'B')
