...
function rysujMape(lista){
  if(!map){
    ...
    // Kontener mapy siedzi w siatce CSS i ma finalny rozmiar dopiero po ulozeniu
    // strony. Bez tego Leaflet liczy zly rozmiar i nie pobiera kafelkow.
  ...
  if(pkt.length){ map.invalidateSize(); map.fitBounds(pkt,{padding:[45,45],maxZoom:9}); }
